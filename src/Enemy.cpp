...
void Enemy::applyStun(float duration) { stunTimer = std::max(stunTimer, duration); }
void Enemy::applyBurn(float dmg, float duration) { burnDamagePerSec = dmg; burnTimer = std::max(burnTimer, duration); }
void Enemy::applySlow(float duration) { slowTimer = std::max(slowTimer, duration); }
